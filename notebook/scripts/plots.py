...
def graph_inc_line(full_data,plot_amount,plot_title,save_fn,tau):
    top_configs = find_best_configs(full_data,plot_amount,tau)
    real_inc = pd.read_csv("../params/stepped_data_7.csv")
    real_inc = list(real_inc["Delta Orchard"])
    months = ["02/11","03/11","04/11","05/11","06/11","07/11","08/11","09/11","10/11","11/11","12/11","01/12","02/12","03/12"]
    fig_x,fig_y = 20,10
    real_mark_size,syn_mark_size = 24,12
    fs = 20
    ts = 24
    plt.figure(figsize=(fig_x, fig_y))
    # go through best configs, and plot
    for config in top_configs:
        inc_data = config[0][1]
        plt.plot(months,inc_data,label=config[0][0],marker='o',markersize=syn_mark_size)
    plt.xticks(fontsize=fs)
    plt.yticks(fontsize=fs)
    plt.title(plot_title,fontsize=ts)
    plt.legend(fontsize=fs)
    plt.plot(real_inc,color='k',marker='o',markersize=real_mark_size)
    plt.savefig(save_fn)
    return
