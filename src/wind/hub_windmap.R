...
lc <- rast("../../data/converted.tif") %>% # load wind time series rasters
...
neighbor_values = read.csv("../../data/hub_neigh_ids.csv")
...
# lc <- lc / 24
example_rose <- rast("../../data/converted.tif") %>%
  wind_series(order = "uuvv") %>%
  wind_rose()
neighbor_values <- read.csv("../../data/hub_neigh_ids.csv")
